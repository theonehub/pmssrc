...
class DeductionSection80CCD:
    ...
    def calculate_80ccd_1_deduction(self, regime: TaxRegime) -> Money:
    ...
    def calculate_80ccd_1b_deduction(self, regime: TaxRegime) -> Money:
    ...
    def calculate_80ccd_2_deduction(self, regime: TaxRegime) -> Money:
...
class DeductionSection80D:
    ...
    def __post_init__(self):
        """Validate ages."""
        if self.employee_age < 18 or self.employee_age > 100:
            raise ValueError("Invalid employee age")
        
        if self.parent_age < 18 or self.parent_age > 120:
            raise ValueError("Invalid parent age")
    ...
    def calculate_eligible_deduction(self, regime: TaxRegime) -> Money:
        ...
        self_family_limit = self.calculate_self_family_limit()
...
class DeductionSection80G:
    ...
    def calculate_category_3_deduction(self) -> Money:
    ...
    def calculate_category_4_deduction(self) -> Money:
    ...
    def calculate_eligible_deduction(self, regime: TaxRegime) -> Money:
        ...
        category_3 = self.calculate_category_3_deduction()
        category_4 = self.calculate_category_4_deduction()
    ...
    def get_tax_saving_suggestions(self) -> Dict[str, Any]:
        """Get suggestions for maximizing 80G tax benefits."""
        qualifying_limit = self.gross_income.percentage(10)
        current_deduction = self.calculate_eligible_deduction(TaxRegime(TaxRegimeType.OLD))
        
        suggestions = []
        
        # Category 3 & 4 combined suggestions (same qualifying limit)
        category_3_current = self.calculate_category_3_deduction()
        category_4_current = self.calculate_category_4_deduction()
        
        combined_used = category_3_current.add(category_4_current)
        
        if combined_used.is_less_than(qualifying_limit):
            remaining_limit = qualifying_limit.subtract(combined_used)
            suggestions.append({
                "category": "Donations with Qualifying Limit",
                "remaining_limit": remaining_limit.to_float(),
                "suggestion": "Consider additional donations to government/charitable institutions",
                "potential_tax_saving": remaining_limit.percentage(30).to_float(),
                "recommended_donations": [
                    "Government donations for charitable purposes",
                    "Family planning donations (100% deduction)",
                    "Housing authority donations",
                    "Religious place renovation"
                ]
            })
        
        # Category 1 & 2 suggestions (no limit)
        category_1_current = self.calculate_category_1_deduction()
        category_2_current = self.calculate_category_2_deduction()
        
        suggestions.append({
            "category": "Unlimited Deduction Donations",
            "current_deduction": category_1_current.add(category_2_current).to_float(),
            "suggestion": "Consider donations to these funds for unlimited deduction",
            "recommended_donations": [
                ...
                "CM Relief Fund (100% deduction)"
            ]
        })
        
        return {
            "total_current_donations": self.calculate_total_donations().to_float(),
            "total_current_deduction": current_deduction.to_float(),
            "qualifying_limit": qualifying_limit.to_float(),
...
class TaxDeductions:
    ...
    def calculate_total_deductions_with_80g_adjustment(self, gross_total_income: Money, regime: TaxRegime) -> Money:
        ...
        if self.section_80g:
            ...
            # Update gross income for 80G calculation
            self.section_80g.gross_income = adjusted_income
    ...
    def get_tax_saving_suggestions(self, regime: TaxRegime, current_income: Money) -> Dict[str, Any]:
        """Enhanced tax saving suggestions."""
        if regime.regime_type == TaxRegimeType.NEW:
            return {
                "message": "Consider switching to old regime if you have significant deductions",
                "suggestions": [],
                "regime_comparison_recommended": True
            }
        
        suggestions = []
        
        # Combined 80C + 80CCC + 80CCD(1) suggestions
        current_combined = self.calculate_combined_80c_80ccc_80ccd1_deduction(regime)
        max_combined_limit = Money.from_int(150000)
        
        if current_combined.is_less_than(max_combined_limit):
            remaining = max_combined_limit.subtract(current_combined)
            suggestions.append({
                "section": "80C/80CCC/80CCD(1)",
                "remaining_limit": remaining.to_float(),
                "potential_tax_saving": remaining.percentage(30).to_float(),
                "investment_options": ["ELSS", "PPF", "NPS", "Life Insurance", "Tax Saving FD"]
            })
        
        # 80CCD(1B) suggestions
        if self.section_80ccd:
            current_1b = self.section_80ccd.calculate_80ccd_1b_deduction(regime)
            max_1b_limit = Money.from_int(50000)
            
            if current_1b.is_less_than(max_1b_limit):
                remaining_1b = max_1b_limit.subtract(current_1b)
                suggestions.append({
                    "section": "80CCD(1B)",
                    "remaining_limit": remaining_1b.to_float(),
                    "potential_tax_saving": remaining_1b.percentage(30).to_float(),
                    "investment_options": ["Additional NPS contribution"]
                })
        
        # 80D suggestions
        if self.section_80d:
            current_80d = self.section_80d.calculate_eligible_deduction(regime)
            max_80d_limit = Money.from_int(100000)  # Max possible (self+parents both senior)
            
            if current_80d.is_less_than(max_80d_limit):
                remaining_80d = max_80d_limit.subtract(current_80d)
                suggestions.append({
                    "section": "80D",
                    "current_deduction": current_80d.to_float(),
                    "additional_possible": remaining_80d.to_float(),
                    "suggestion": "Consider health insurance for family/parents"
                })
        
        return {
            "total_current_deductions": self.calculate_total_deductions(regime).to_float(),
            "total_current_exemptions": self.calculate_interest_exemptions(regime).to_float(),
            "suggestions": suggestions
        } 