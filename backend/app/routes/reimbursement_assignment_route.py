...
@router.post("/assign")
def assign_reimbursements(payload: ReimbursementAssignmentCreate):
    assign_reimbursements_to_user(payload)
    return {"msg": "Reimbursements assigned successfully"}

@router.get("/user/{emp_id}", dependencies=[Depends(get_current_user)])
def get_user_assignments(emp_id: str):
    data = get_user_reimbursement_assignments(emp_id)
    return data or {}
...
def get_all_assignments(
    ...
    return get_all_user_reimbursement_assignments(skip, limit, search)