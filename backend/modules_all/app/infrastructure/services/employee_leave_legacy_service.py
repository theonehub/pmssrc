...
def get_working_days(start_date, end_date, hostname: str):
    ...
    while current_date <= end:
        ...
        if not is_weekend(current_date) and not is_public_holiday_sync(current_date.strftime("%Y-%m-%d"), hostname):
            working_days += 1
...
def calculate_lwp_for_month(employee_id: str, month: int, year: int, hostname: str):
    ...
    try:
        ...
        while current_date <= month_end:
            ...
            if is_weekend(current_date) or is_public_holiday_sync(current_date.strftime("%Y-%m-%d"), hostname):
                current_date += timedelta(days=1)
                continue
