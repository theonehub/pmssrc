...
const AllLeaves: React.FC = () => {
  ...
  const fetchLeaves = async (): Promise<void> => {
    try {
      const response = await axios.get('/api/v2/leaves/all');
  ...
  const fetchLeaveStats = async (): Promise<void> => {
    try {
      const response = await axios.get('/api/v2/leaves/stats');
  ...
  const handleConfirm = async (): Promise<void> => {
    ...
    try {
      await axios.put(
        `/api/v2/leaves/${selectedLeave.leave_id}/status?status=${selectedLeave.action}`
      );
      toast.success(`Leave ${selectedLeave.action} successfully`);
      fetchLeaves();
      fetchLeaveStats(); // Refresh stats after status change
      setOpenDialog(false);
    } catch (error: any) {
      if (process.env.NODE_ENV === 'development') {
        // eslint-disable-next-line no-console
