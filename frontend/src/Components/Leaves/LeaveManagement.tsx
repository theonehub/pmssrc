...
import { LeaveRequest, LeaveBalanceData, AlertState } from '../../types';
...
const LeaveManagement: React.FC = () => {
  ...
  const fetchLeaveBalance = async (): Promise<void> => {
    try {
      const response = await api.get('/api/v2/leaves/leave-balance');
  ...
  const fetchLeaves = async (): Promise<void> => {
    try {
      const response = await api.get('/api/v2/leaves/my-leaves');
  ...
  const handleSubmit = async (e: React.FormEvent): Promise<void> => {
    ...
    try {
      ...
      if (editingLeave) {
        await api.put(`/api/v2/leaves/${editingLeave._id}`, leaveData);
        setAlert({
          open: true,
        ...
        await api.post('/api/v2/leaves/apply', leaveData);
        setAlert({
          open: true,
