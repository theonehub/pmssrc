...
const TaxationDashboard: React.FC = () => {
  ...
  const handleViewDetails = (empId: string | undefined): void => {
    if (!empId) return;
    navigate(`/taxation/details/${empId}`);
  };
  ...
  // Helper function to safely format filing status
    ...
    return (
      <TableContainer component={Paper} sx={{ mt: 2 }}>
        <Table sx={{ minWidth: 650 }} aria-label="taxation table">
          ...
          <TableBody>
            {taxationData.map((row, index) => (
              <TableRow key={`${row.employee_id || 'unknown'}-${row.tax_year || 'unknown'}-${index}`}>
                <TableCell>{row.employee_id || 'N/A'}</TableCell>
                <TableCell>{row.tax_year || 'N/A'}</TableCell>
                ...
                <TableCell>
                  <Box sx={{ 
                    ...
                    {formatFilingStatus(row.filing_status)}
                  </Box>
                </TableCell>
                <TableCell>
                  <Button 
              ...
              <Table size="small">
                ...
                <TableBody>
                  ...
                  <TableRow>
                    ...
                    <TableCell align="right">
                      {formatCurrency(
                        typeof userData.capital_gains === 'object' && userData.capital_gains ? (
