  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (employee_id && month && year) {
        const response = await apiClient.get(`/api/v2/attendance/employee/${employee_id}/month/${month}/year/${year}`);
        return response.data;
      }
      
      // If we have employee_id and year only, use year endpoint
      if (employee_id && year && !month) {
        const response = await apiClient.get(`/api/v2/attendance/employee/${employee_id}/year/${year}`);
        return response.data;
      }
      
      // If we have month and year but no employee_id, use "my" endpoints (current user)
      if (month && year && !employee_id) {
        const response = await apiClient.get(`/api/v2/attendance/my/month/${month}/year/${year}`);
        return response.data;
      }
      ...
      if (date && month && year && !employee_id) {
        const response = await apiClient.get(`/api/v2/attendance/team/date/${date}/month/${month}/year/${year}`);
        return response.data;
      }
      
      // For team/manager views with month
      if (month && year && !employee_id && !date) {
        const response = await apiClient.get(`/api/v2/attendance/team/month/${month}/year/${year}`);
        return response.data;
      }
